# coding:utf-8
#此页完成了对http://www.ahzfcg.gov.cn/cmsNewsController/getCgggNewsList.do?channelCode=cggg&bid_type=011&pageNum=2 安微省政府采购网
#此网站对ip有一定的限制
...
#url = 'http://www.ahzfcg.gov.cn/cmsNewsController/getCgggNewsList.do?channelCode=cggg&bid_type=011&pageNum=2'
url = 'http://www.ahzfcg.gov.cn/cmsNewsController/getCgggNewsList.do?channelCode=cggg&bid_type=011&pageNum='
...
for url in urls:
    ...
    #print soup.prettify()
    links = soup.find_all('table',border=re.compile("0"))
    print links
    soup_code = BeautifulSoup(str(links), 'lxml')
    pages = soup_code.find_all('a', title=re.compile(""))
    print pages
    for page in pages:
        #print page
        time.sleep(3)
        ...
        sub_page_url = sub_page_url    #截取子页面的地址
        print sub_page_url
        ...
        #获取标题
        sub_links_title = sub_soup.find('div', class_=re.compile("frameNews")).find('h1')
        #获取发布时间
        sub_date = sub_soup.find('div',class_=re.compile('source')).find('span')
        print sub_date
        title = sub_links_title.get_text().strip()
        print title
        date_time = sub_date.get_text().strip()
        date_time = date_time[5:15]
        print date_time
        # 发布时间为 date_time
        ...
        if date_time < '2018-10-01':
            print '-------date not match-------'
            continue

        # 获取内容详情
        sub_links_content = sub_soup.find_all('div', class_=re.compile("con"))
        #print date_time
